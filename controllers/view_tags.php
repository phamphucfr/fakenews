...
<?php $listeActus = ManagerActus::findByTag($cnx, (int)$tag_selected->getIdTag()); ?>
<table>
...
<?php foreach($listeTags as $tag) { ?>    
  <a href="index.php?view=2&tag=<?= $tag->getIdTag(); ?>" 
     class="<?= ($tag->getIdTag()==$tag_selected->getIdTag())?'selected':''?>" >#<?= $tag->getNameTag(); ?>
  </a>
<?php } ?>     
